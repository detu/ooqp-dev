...
{
	double primalValue, primalStep, dualValue, dualStep, maxAlpha, mufull;
	int    firstOrSecond;

	maxAlpha =
	    iterate->findBlocking(step, primalValue, primalStep, dualValue, dualStep, firstOrSecond);
...
int Solver::doStatus(Data* data, Variables* vars, Residuals* resids, int i, double mu, int level)
{
	if (status)
	{
                          ...
                          int /* level */)
{
	...
	if (rnorm / dnorm > artol &&
	    (rnorm_history[idx] / mu_history[idx]) / (rnorm_history[0] / mu_history[0]) >= 1.e8)
		stop_code = UNKNOWN;
