...
void QpGenResiduals::add_r3_xz_alpha(Variables* vars_in, double alpha)
{
	...
	if (mclow > 0)
		rlambda->axzpy(1.0, *vars->t, *vars->lambda);
	if (mcupp > 0)
		rpi->axzpy(1.0, *vars->u, *vars->pi);
	if (nxlow > 0)
		rgamma->axzpy(1.0, *vars->v, *vars->gamma);
	if (nxupp > 0)
		rphi->axzpy(1.0, *vars->w, *vars->phi);
...
int QpGenResiduals::validNonZeroPattern()
{
	...
	if (mclow > 0 &&
	    (!rt->matchesNonZeroPattern(*iclow) || !rlambda->matchesNonZeroPattern(*iclow)))
	{
		return 0;
	}
