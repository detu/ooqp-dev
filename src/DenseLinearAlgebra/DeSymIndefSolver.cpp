...
extern "C" void
dsytrf_(char* uplo, int* n, double A[], int* lda, int ipiv[], double work[], int* lwork, int* info);
...
void DeSymIndefSolver::matrixChanged()
{
	...
	dsytrf_(&fortranUplo, &n, &mStorage->M[0][0], &n, ipiv, work->elements(), &lwork, &info);
}
...
void DeSymIndefSolver::solve(OoqpVector& v)
{
	...
	dsytrs_(&fortranUplo, &n, &one, &mStorage->M[0][0], &n, ipiv, &sv[0], &n, &info);
}
