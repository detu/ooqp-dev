...
#include "src/SparseLinearAlgebra/SparseStorage.h"
#include "src/SparseLinearAlgebra/SparseSymMatrix.h"
...
Ma27Solver::Ma27Solver(SparseSymMatrix* sgm) : Ma27SolverBase(sgm->size(), sgm->numberOfNonZeros())
{
	SpReferTo(mMat, sgm);
}
...
void Ma27SolverBase::firstCall()
{
	...
	double ops;
	ma27ad_(&n, &nnz, irowM, jcolM, iw, &liw, ikeep, iw1, &nsteps, &iflag, icntl, cntl, info, &ops);
	...
	switch (this->ma27ErrFlg())
	{
		...
		case -3:
		{
			if (gOoqpPrintLevel >= 100)
			{
				std::cout << "insufficient space in iw: " << liw << " suggest reset to "
		...
		case 1:
		{
			std::cerr << "detected " << this->ierror()
			          << " entries out of range in irowM and jcolM; ignored" << std::endl;
		};
		break;
	}
...
void Ma27SolverBase::matrixChanged()
{
	...
	do
	{
...
#endif
		ma27bd_(&n, &nnz, irowM, jcolM, fact, &la, iw, &liw, ikeep, &nsteps, &maxfrt, iw1, icntl,
		        cntl, info);
#ifdef HAVE_GETRUSAGE
		rusage after;
		if (print_level >= 100)
		{
			...
			cout << "For try " << tries + 1 << " the factorization took "
		...
		switch (this->ma27ErrFlg())
		{
			...
			case -3:
			{
				...
				delete[] iw;
				liw =
				    (this->ierror() > ipessimism * liw) ? this->ierror() : (int)(ipessimism * liw);
				iw = new int[liw]();
				if (print_level >= 100)
			...
			case -4:
			{
				...
				delete[] fact;
				la   = (this->ierror() > rpessimism * la) ? this->ierror() : (int)(rpessimism * la);
				fact = new double[la]();
				this->copyMatrixElements(fact, la);
				if (print_level >= 100)
			...
			case -5:
			{
				if (print_level >= 100)
				{
					std::cout << "matrix apparently numerically singular, detected at stage "
					          << this->ierror() << std::endl;
			...
			case -6:
			{
				if (print_level >= 100)
				{
					std::cout << "change of sign of pivots detected at stage " << this->ierror()
					          << std::endl;
...
#endif
	ma27cd_(&nn, fact, &la, iw, &liw, w, &maxfrt, drhs, iw1, &nsteps, icntl, info);
#ifdef HAVE_GETRUSAGE
	rusage after;
	if (print_level >= 100)
	{
...
void Ma27Solver::solve(OoqpVector& rhs_in)
{
	...
	while (!done && refactorizations < 10)
	{
		...
		else if (this->thresholdPivoting() >= kThresholdPivotingMax || refactorizations > 10)
		{
		...
		else
		{
			...
			if (tp > kThresholdPivotingMax)
				tp = kThresholdPivotingMax;
			this->setThresholdPivoting(tp);

			if (gOoqpPrintLevel >= 100)
			{
				std::cout << "Setting ThresholdPivoting parameter to " << this->thresholdPivoting()
				          << " for future factorizations" << std::endl;
			}
			this->matrixChanged();
			refactorizations++;
