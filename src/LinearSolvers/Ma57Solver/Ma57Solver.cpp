...
void Ma57Solver::firstCall()
{
	...
	ma57ad_(&n, &nnz, irowM, jcolM, &lkeep, keep, iwork, icntl, info, rinfo);
...
void Ma57Solver::matrixChanged()
{
	...
	do
	{
		...
		ma57bd_(&n, &nnz, M, fact, &lfact, ifact, &lifact, &lkeep, keep, iwork, icntl, cntl, info,
		        rinfo);
#ifdef HAVE_GETRUSAGE
		rusage after;
		if (gOoqpPrintLevel >= 100)
		...
		if (info[0] != 0)
			std::cout << "Factorization info: " << info[0] << std::endl;
		switch (info[0])
		{
			case 0:
				done = 1;
			...
			case -3:
			{
				...
				ma57ed_(&n, &ic, keep, fact, &lfact, newfact, &lnfact, ifact, &lifact, ifact,
				        &lifact, info);
				delete[] fact;
				fact  = newfact;
				lfact = lnfact;
			...
			case -4:
			{
				...
				ma57ed_(&n, &ic, keep, fact, &lfact, fact, &lfact, ifact, &lifact, nifact, &lnifact,
				        info);
				delete[] ifact;
				ifact  = nifact;
				lifact = lnifact;
...
void Ma57Solver::solve(OoqpVector& rhs_in)
{
	...
	{
		ma57dd_(&job, &n, &nnz, M, irowM, jcolM, fact, &lfact, ifact, &lifact, drhs, dx, dresid,
		        dwork, iwork, icntl, cntl, info, rinfo);
		...
		else
		{
			...
			if (freshFactor)
			{  // We weren't doing iterative refinement,
				// let's do so
				job      = 2;
				icntl[8] = 10;
				...
				else
				{
					...
					this->setThresholdPivoting();
					std::cout << "Setting ThresholdPivoting parameter to " << kThresholdPivoting
			...
			else
			{
				...
				std::cout << "Refactoring with Threshold Pivoting parameter" << kThresholdPivoting
				          << std::endl;
				this->matrixChanged();
				refactorizations++;
