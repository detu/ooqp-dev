...
QpBoundData::QpBoundData(QpBound*    f,
                         ...
                         SymMatrix*  Q,
                         OoqpVector* l_in,
                         OoqpVector* index_lower_in,
                         OoqpVector* u_in,
                         OoqpVector* index_upper_in)
{
	alpha   = alpha_;
	factory = f;
	SpReferTo(c, c_in);
...
void QpBoundData::Qmult(double b, OoqpVector& y, double a, OoqpVector& x)
{
	mQ->mult(b, y, a, x);
}
...
{
	std::cout << "********** Q ******* \n";
	mQ->writeToStream(std::cout);

	std::cout << "*********** c **********\n";
	c->writefToStream(std::cout, "%{value}");

	std::cout << "*********** lower **********\n";
	lower->writefSomeToStream(std::cout, "component %{index} = %{value}", *index_lower);

	std::cout << "*********** upper **********\n";
	upper->writefSomeToStream(std::cout, "component %{index} = %{value}", *index_upper);
}
