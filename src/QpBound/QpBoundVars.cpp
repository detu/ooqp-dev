...
double QpBoundVars::mustep(Variables* step_in, double alpha)
{
	...
	temp = t->shiftedDotProductWith(alpha, *step->t, *tau, alpha, *step->tau);

	temp += v->shiftedDotProductWith(alpha, *step->v, *nu, alpha, *step->nu);
...
void QpBoundVars::saxpy(Variables* b_in, double alpha)
{
	...
	assert(b->v->matchesNonZeroPattern(*index_upper) && b->nu->matchesNonZeroPattern(*index_upper));
	v->axpy(alpha, *b->v);
	nu->axpy(alpha, *b->nu);
}
...
double QpBoundVars::stepbound(Variables* b_in)
{
	...
	assert(t->somePositive(*index_lower) && tau->somePositive(*index_lower));
	assert(v->somePositive(*index_upper) && nu->somePositive(*index_upper));
                                 ...
                                 int&       firstOrSecond)
{
	...
	bound = t->findBlocking(*d->t, *tau, *d->tau, bound, &primalValue, &primalStep, &dualValue,
	                        &dualStep, firstOrSecond);
	bound = v->findBlocking(*d->v, *nu, *d->nu, bound, &primalValue, &primalStep, &dualValue,
	                        &dualStep, firstOrSecond);
	return bound;
}
...
{
	std::cout << std::endl << "Computed Solution x (Primal):" << std::endl;
	x->writefToStream(std::cout, "%{index}: x= %{value}");

	std::cout << std::endl << "Lower Bound Multipliers:" << std::endl;
	tau->writefSomeToStream(std::cout, "tau[%{index}] = %{value}", *index_lower);

	std::cout << std::endl << "Upper Bound Multipliers:" << std::endl;
	nu->writefSomeToStream(std::cout, "nu[%{index}] = %{value}", *index_upper);
}
