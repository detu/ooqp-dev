...
#include "QpGenVars.h"
#include "src/DenseLinearAlgebra/DeSymIndefSolver.h"
#include "src/DenseLinearAlgebra/DenseGenMatrix.h"
#include "src/DenseLinearAlgebra/DenseLinearAlgebraPackage.h"
#include "src/DenseLinearAlgebra/DenseSymMatrix.h"
#include "src/Vector/SimpleVector.h"
#include "src/Vector/SimpleVectorHandle.h"

QpGenDense::QpGenDense(int nx_, int my_, int mz_) : QpGen(nx_, my_, mz_)
{
	la = DenseLinearAlgebraPackage::soleInstance();
}

QpGenDense::QpGenDense(int nx_, int my_, int mz_, int /* nnzQ */, int /* nnzA */, int /* nnzC */)
    : QpGen(nx_, my_, mz_)
{
	la = DenseLinearAlgebraPackage::soleInstance();
}
...
QpGenData* QpGenDense::makeData(double c[],
                                double Q[],
                                double xlow[],
                                char   ixlow[],
                                double xupp[],
                                ...
                                char   icupp[])
{
	...
	return new QpGenData(la, vc, mQ, vxlow, vixlow, vxupp, vixupp, mA, vbA, mC, vclow, viclow,
	                     vcupp, vicupp);
}
...
void QpGenDense::joinRHS(OoqpVector& rhs_in,
                         OoqpVector& rhs1_in,
                         OoqpVector& rhs2_in,
                         OoqpVector& rhs3_in)
{
...
void QpGenDense::separateVars(OoqpVector& x_in,
                              OoqpVector& y_in,
                              OoqpVector& z_in,
                              OoqpVector& vars_in)
{
...
{
	data = new QpGenData(la, nx, my, mz, nx * nx, nx * my, nx * mz);
