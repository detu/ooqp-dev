                               ...
                               int&       firstOrSecond)
{
	...
	if (mclow > 0)
	{
		alpha = t->findBlocking(*d->t, *lambda, *d->lambda, alpha, &primalValue, &primalStep,
		                        &dualValue, &dualStep, firstOrSecond);
	}

	if (mcupp > 0)
	{
		alpha = u->findBlocking(*d->u, *pi, *d->pi, alpha, &primalValue, &primalStep, &dualValue,
		                        &dualStep, firstOrSecond);
	}

	if (nxlow > 0)
	{
		alpha = v->findBlocking(*d->v, *gamma, *d->gamma, alpha, &primalValue, &primalStep,
		                        &dualValue, &dualStep, firstOrSecond);
	}

	if (nxupp > 0)
	{
		alpha = w->findBlocking(*d->w, *phi, *d->phi, alpha, &primalValue, &primalStep, &dualValue,
		                        &dualStep, firstOrSecond);
	}
...
{
	x->writefToStream(std::cout, "x[%{index}] = %{value}");
}
