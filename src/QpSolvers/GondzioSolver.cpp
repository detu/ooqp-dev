...
#include "src/Abstract/Data.h"
#include "src/Abstract/LinearSystem.h"
#include "src/Abstract/ProblemFormulation.h"
#include "src/Abstract/Residuals.h"
#include "src/Abstract/Status.h"
#include "src/Abstract/Variables.h"
...
GondzioSolver::GondzioSolver(ProblemFormulation* of, Data* prob) : NumberGondzioCorrections(0)
{
	factory         = of;
...
int GondzioSolver::solve(Data* prob, Variables* iterate, Residuals* resid)
{
	...
	do
	{
		...
		{
			this->doMonitor(prob, iterate, resid, alpha, sigma, iter, mu, status_code, 0);
		}
		// *** Predictor step ***
		...
		if (gOoqpPrintLevel >= 10)
		{
			this->doMonitor(prob, iterate, resid, alpha, sigma, iter, mu, status_code, 2);
		}
		...
		while (NumberGondzioCorrections < maximum_correctors && alpha < 1.0 && !StopCorrections)
		{
...
void GondzioSolver::defaultMonitor(Data* /* data */,
                                   Variables* /* vars */,
                                   Residuals* resids,
                                   double     alpha,
                                   double     sigma,
                                   ...
                                   int        level)
{
	switch (level)
	{
		...
		case 1:
		{
			std::cout << std::endl << "Duality Gap: " << resids->dualityGap() << std::endl;
			if (i > 1)
			{
			...
			std::cout << " mu = " << mu
			          << " relative residual norm = " << resids->residualNorm() / dnorm
			          << std::endl;
