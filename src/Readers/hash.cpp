...
#define NUMPRIMES 18
static int prime[NUMPRIMES] = {29,     229,     883,     1669,    2791,    4801,
                               8629,   15289,   32749,   65521,   131071,  262139,
...
extern "C" char* StrDup(char* s1, const char* message)
{
	int   ls1 = strlen(s1) + 1;
...
extern "C" HashTable* NewHashTable(int size)
{
	int        i;
	HashTable* table = 0;
...
extern "C" int hash(HashTable* table, char* string)
{
...
extern "C" int GetIndex(HashTable* table, char* name)
{
...
extern "C" int Insert(HashTable* table, char* name, int index)
{
	List* ptr;
	int   i;
...
extern "C" int PrintHashTable(HashTable* table)
{
	int     i;
	ListPtr ptr;
...
extern "C" int DeleteHashTable(HashTable* table)
{
	int     i;
	ListPtr ptr;
...
extern "C" int PrintHashTableStats(HashTable* table)
{
	int     i, count, max = 0;
	ListPtr ptr;
