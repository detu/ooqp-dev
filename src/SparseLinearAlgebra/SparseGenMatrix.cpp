...
SparseGenMatrix::SparseGenMatrix(int rows, int cols, int nnz, int krowM[], int jcolM[], double M[])
{
	mStorage = SparseStorageHandle(new SparseStorage(rows, cols, nnz, krowM, jcolM, M));
}
...
void SparseGenMatrix::mult(double beta, OoqpVector& y_in, double alpha, OoqpVector& x_in)
{
...
void SparseGenMatrix::transMult(double beta, OoqpVector& y_in, double alpha, OoqpVector& x_in)
{
	SimpleVector& x = dynamic_cast<SimpleVector&>(x_in);
	SimpleVector& y = dynamic_cast<SimpleVector&>(y_in);
