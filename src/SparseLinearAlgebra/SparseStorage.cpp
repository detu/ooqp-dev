...
SparseStorage::SparseStorage(int m_, int n_, int len_, int* krowM_, int* jcolM_, double* M_)
{
	neverDeleteElts = 1;
	m               = m_;
	n               = n_;
...
void SparseStorage::fromGetDense(int row, int col, double* A, int lda, int rowExtent, int colExtent)
{
	int i, j, k;
...
void SparseStorage::atPutSpRow(int row, double A[], int lenA, int jcolA[], int& info)
{
	int ka    = lenA - 1;
...
void SparseStorage::atPutDense(int row, int col, double* A, int lda, int rowExtent, int colExtent)
{
	int info;
	int i;
...
void SparseStorage::putSparseTriple(int irow[], int lenA, int jcol[], double A[], int& info)
{
	if (len < lenA)
	{
		info = 1;
	...
	for (; kinc >= 0; kinc--)
	{
			...
			for (k = inc; k < n; k++)
			{
				...
				for (j = k; j >= inc; j -= inc)
				{
					...
					if (fi < first[index[j - inc]] ||
					    (fi == first[index[j - inc]] && se < second[index[j - inc]]))
					{
		...
		else if (swapSecond && !swapFirst)
		{
			for (k = inc; k < n; k++)
			{
				...
				{
					if (fi < first[index[j - inc]] ||
					    (fi == first[index[j - inc]] && se < second[j - inc]))
					{
						index[j]  = index[j - inc];
		...
		else if (swapFirst && !swapSecond)
		{
			for (k = inc; k < n; k++)
			{
				...
				{
					if (fi < first[j - inc] ||
					    (fi == first[j - inc] && se < second[index[j - inc]]))
					{
						index[j] = index[j - inc];
						first[j] = first[j - inc];
					}
...
void SparseStorage::mult(double beta, double y[], int incy, double alpha, double x[], int incx)
{
	int    i, j, k;
	double temp;
	if (beta == 0)
...
void SparseStorage::transMult(double beta, double y[], int incy, double alpha, double x[], int incx)
{
	int i, j, k;
	if (beta == 0)
	{
                                    ...
                                    int irowM[])
{
	...
	indexedLexSort(jcolM, kout, dontPermuteCols, irowM, doPermuteRows, kpat);
...
void SparseStorage::atPutDiagonal(int idiag, double x[], int incx, int extent)
{
	int i;
	int info;
	for (i = idiag; i < idiag + extent; i++)
	{  // Loop over elts to be put
		// Search for the diagonal elt.
		int lastk;
		lastk = krowM[i + 1];
		for (int k = krowM[i]; k < lastk; k++)
		{  // Loop over all elts in row
			if (i >= jcolM[k])
			{  // Found or past the diagonal
				if (i == jcolM[k])
