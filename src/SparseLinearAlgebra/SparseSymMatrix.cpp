...
#include "src/Abstract/DoubleMatrixTypes.h"
...
SparseSymMatrix::SparseSymMatrix(int size, int nnz, int krowM[], int jcolM[], double M[])
{
	mStorage = SparseStorageHandle(new SparseStorage(size, size, nnz, krowM, jcolM, M));
}
...
void SparseSymMatrix::fromGetDense(int     row,
                                   int     col,
                                   double* A,
                                   int     lda,
                                   int     rowExtent,
...
void SparseSymMatrix::fromGetSpRow(int    row,
                                   int    col,
                                   double A[],
                                   int    lenA,
                                   int    jcolA[],
...
void SparseSymMatrix::symAtPutSubmatrix(int           destRow,
                                        int           destCol,
                                        DoubleMatrix& M,
                                        int           srcRow,
                                        int           srcCol,
...
void SparseSymMatrix::mult(double beta, OoqpVector& y_in, double alpha, OoqpVector& x_in)
{
...
void SparseSymMatrix::transMult(double beta, OoqpVector& y_in, double alpha, OoqpVector& x_in)
{
	SimpleVector& x = dynamic_cast<SimpleVector&>(x_in);
	SimpleVector& y = dynamic_cast<SimpleVector&>(y_in);
...
void SparseSymMatrix::transMult(double beta,
                                double y[],
                                int    incy,
                                double alpha,
                                double x[],
...
void SparseSymMatrix::mult(double beta, double y[], int incy, double alpha, double x[], int incx)
{
	int m, n, i, j, k;
	this->getSize(m, n);
