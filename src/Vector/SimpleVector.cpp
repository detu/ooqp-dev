...
#include "src/Utilities/OoqpBlas.h"
...
SimpleVector::SimpleVector(double* v_, int n_) : OoqpVector(n_)
{
	preserveVec = 1;
	v           = v_;
}
...
void SimpleVector::writefToStream(std::ostream& out, const char format[]) const
{
...
void SimpleVector::axzpy(double alpha, OoqpVector& xvec, OoqpVector& zvec)
{
	assert(n == xvec.length() && n == zvec.length());
...
void SimpleVector::axdzpy(double alpha, OoqpVector& xvec, OoqpVector& zvec)
{
	SimpleVector& sxvec = dynamic_cast<SimpleVector&>(xvec);
	double*       x     = sxvec.v;
	SimpleVector& szvec = dynamic_cast<SimpleVector&>(zvec);
...
void SimpleVector::axdzpy(double alpha, OoqpVector& xvec, OoqpVector& zvec, OoqpVector& select)
{
	assert(n == xvec.length() && n == zvec.length());
...
double SimpleVector::shiftedDotProductWith(double      alpha,
                                           ...
                                           OoqpVector& ystep)
{
	assert(n == mystep.length() && n == yvec.length() && n == ystep.length());
                                  ...
                                  int&        first_or_second)
{
	...
	return ::find_blocking(w, n, 1, wstep, 1, u, 1, ustep, 1, maxStep, w_elt, wstep_elt, u_elt,
	                       ustep_elt, first_or_second);
}
